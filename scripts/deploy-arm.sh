#!/usr/bin/env bash
# Deploy pre-built pertisk-proxy packages from release/ (ARM64 RPM).
# Run ./scripts/build.sh first (or set PACKAGE_BUILD=1 to build during deploy).
#
# Usage (from repo root):
#   DEPLOY_HOST=user@proxy.example.com ./scripts/deploy-arm.sh
set -euo pipefail
...
echo "==> Deploying v${VERSION} (arm64) to ${DEPLOY_HOST} (PACKAGE_BUILD=${PACKAGE_BUILD})"
deploy deploy-rpm-arm64 DEPLOY_HOST="${DEPLOY_HOST}"
