#!/usr/bin/env bash
# Package pertisk-ingress Helm chart and upload to the Pertisk chart repository.
#
# Usage:
#   VERSION=0.1.0 ./build/publish-helm-ingress.sh
...
CHART_TGZ="$WORKDIR/pertisk-ingress-${VERSION}.tgz"
...
if [ "$HTTP_CODE" -ge 200 ] && [ "$HTTP_CODE" -lt 300 ]; then
  ...
  echo ""
  echo "Published pertisk-ingress ${VERSION} (image: harbor.homelab.pertisk.com/pertisksoft/pertisk-proxy/ingress:v${VERSION})"
